    ...
    for vol_path in common.get_file_paths(first_stage_reg_dir, ignore_folders=[RESOLUTION_IMGS_DIR, IMG_PYRAMID_DIR]):
        ...
        if label_path.is_file():
            ...
            if mask is None: # get a few slices from middle
                sag_indxs = np.linspace(0, arr.shape[2], 16, dtype=np.int)[4:-4]
            else:
                sag_start = bbox[2]
                sag_end = bbox[5]
            ...
            if mask is None: # get a few slices from middle
                ax_indxs = np.linspace(0, arr.shape[0], 16, dtype=np.int)[4:-4]
            else:
                ax_start = bbox[0]
                ax_end = bbox[3]
            ...
            if mask is None: # get a few slices from middle
                cor_indxs = np.linspace(0, arr.shape[1], 16, dtype=np.int)[4:-4]
            else:
                cor_start = bbox[1]
                cor_end = bbox[4]
